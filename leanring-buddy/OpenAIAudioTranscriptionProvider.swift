...
final class OpenAIAudioTranscriptionProvider: BuddyTranscriptionProvider {
    /// Proxy route on the Cloudflare Worker. This keeps the OpenAI API key
    /// off-device while still letting the app use OpenAI transcription.
    private static let transcriptionProxyURL = "https://clicky-proxy.akash8860biswas.workers.dev/transcribe"
    ...
    func startStreamingSession(
        ...
        return OpenAIAudioTranscriptionSession(
            transcriptionProxyURL: Self.transcriptionProxyURL,
            modelName: modelName,
            keyterms: keyterms,
...
private final class OpenAIAudioTranscriptionSession: BuddyStreamingTranscriptionSession {
    ...
    private let stateQueueSpecificKey = DispatchSpecificKey<Void>()
    ...
    init(
        ...
        self.transcriptionProxyURL = URL(string: transcriptionProxyURL)!
        self.modelName = modelName
        self.keyterms = keyterms
        self.onTranscriptUpdate = onTranscriptUpdate
        self.onFinalTranscriptReady = onFinalTranscriptReady
        self.onError = onError
        self.stateQueue.setSpecific(key: stateQueueSpecificKey, value: ())
    ...
    func cancel() {
        ...
        if DispatchQueue.getSpecific(key: stateQueueSpecificKey) != nil {
            ...
            stateQueue.sync(execute: updateCancellationState)
        }
    ...
    private func requestTranscription(for wavAudioData: Data) async throws -> String {
        ...
        guard let httpResponse = response as? HTTPURLResponse else {
            throw OpenAIAudioTranscriptionProviderError(
                message: "OpenAI transcription proxy returned an invalid response."
            )
        }
